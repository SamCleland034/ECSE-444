...
int main() {
	...
	float varianceC;
	float32_t varianceLibFunc;
	float resultASM;
	float varianceASM;
	float resultC;


	resultC = dotProduct(f1000_array, f1000_array, size);
	printf("Pure C: Dot Product = %f\n", resultC);
	arm_dot_prod_f32(f1000_array, f1000_array, size, &resultLibFunc);
	printf("Library Function: Dot Product = %f\n", resultLibFunc);
	asm_dot_product(f1000_array, f1000_array, size, &resultASM);
	printf("Assembly Subroutine: Dot Product = %f\n", resultASM);
	varianceC = variance(f1000_array, size);
